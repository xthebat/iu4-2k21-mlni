    ...
    if data[y][x] == '1':
        ...
        if ways_number == 1 or ways_number > 2:
            ...
            if [x, y] not in nodes.values():
                ...
                print(f'Этот уже был под id: {point_id}')
                ways = []
            # True - это узел
...
def find_nodes(data):
    ...
    last_y = len(data) - 1
    last_x = len(data[0]) - 1
    print(f'Последний индекс по Х: {last_x}')
    print(f'Последний индекс по Y: {last_y}')
...
def recursive_explorer(dc):
    ...
    for node in dc.current_ways:
        ...
        for way in node.ways_to_explore:
            ...
            print(f'Найден узел: {dc.point.vec}')
...
class Maze:
    ...
    def load_from_txt(self, txt_path):
        ...
        print(self.adj)
        self.adj_matrix = adjacency_matrix(self.nodes, self.adj)

    def to_json(self):
        f = open('maze_graph.json', 'wt')
